...
def writecsv(fold, conf, perf, sdr, util):
    '''
    ...
    print "\033[40;36m IP\t\tSMT/Vcore/Freq\tMR#\tMR_RT\tSQL#\tSQL_RT\tMR+SQL#\t#vsRef\tMRSQL/W\t/WvsRef\tMaxPowr\tAcuVcor\tUser%\tSys%\tIdle%\tJDK \033[0m"
    
    # Get Ref server data for compare
    ref_tput = 0
    if  perf.get("100.76.29.3"):
    ...
    for ser in sorted(fold.values()):
        ...
        datatable[row][13] = perf[ser][0] if perf[ser][0] else 0  # MR#
        datatable[row][14] = perf[ser][1].split(' ')[0] if perf[ser][1] else 0  # MR Time
        datatable[row][15] = ""   # Spark # (SQL+Scala)
        datatable[row][16] = ""   # Spark Time
        datatable[row][17] = perf[ser][2] if perf[ser][2] else 0   # SQL#
        datatable[row][18] = perf[ser][3].split(' ')[0] if perf[ser][3].split(' ')[0] else 0  # SQL Time
        datatable[row][19] = perf[ser][4] if perf[ser][4] else 0  # Scala
        datatable[row][20] = perf[ser][5].split(' ')[0] if perf[ser][5] else 0  # Scala Time
        datatable[row][21] = float(sdr[ser][0].split()[0]) if sdr[ser] else 0   # Avg Power
        datatable[row][22] = float(sdr[ser][0].split()[1]) if sdr[ser] else 0  # Max Power
        datatable[row][23] = float(sdr[ser][0].split()[2]) if sdr[ser] else 0  # Min Power
        datatable[row][24] = float(sdr[ser][0].split()[3]) if sdr[ser] else 0  # Stdev Power
        datatable[row][25] = ""   # Topops
        datatable[row][26] = round(float(util[ser][0]))   # Actual Vcore#
        datatable[row][27] = float(util[ser][1])/100   # CPU User%
        datatable[row][28] = float(util[ser][2])/100   # CPU Sys%
        datatable[row][29] = float(util[ser][3])/100   # CPU Idle%
        ...
        actualvcore = datatable[row][26]
        user_cpu = datatable[row][27]
        sys_cpu = datatable[row][28]
        idle_cpu = datatable[row][29]
        ...
        print "\t{mrsql_nbr}\t{mrsql_nbr_vs_ref}x\t{mrsql_perwatt}\t{mrsql_perwatt_vs_ref}x\t{maxpower}\t{actualvcore}\t{usercpu}%\t{syscpu}%\t{idlecpu}%\t{jdk}".\
            ...
            maxpower=int(datatable[row][22]), actualvcore=int(actualvcore), usercpu=user_cpu*100, syscpu=sys_cpu*100, idlecpu=idle_cpu*100, jdk=str(conf[ser][8].split(': ')[1])[-20:])
        row += 1
...
if __name__ == '__main__':
    ...
    with open("tmpcpu.txt") as f:
        for line in f:
            ...
            list.append(line.split("\t")[4].strip())
            util[line.split("\t")[0]] = list
    ...
    writecsv(fold, confret, perfret, sdrret, util)
    '''
    # write power distribution
    powerdist = {}
    with open("tmppower.txt") as f:
        for line in f:
            for i in range(5):

            list = []
            list[i].append(line.split(" ")[1])
            list[i].append(line.split(" ")[2])
            
            powerdist[line.split(" ")[0]] = list
